...
class ExperimentRunner:
    ...
    def start_experiment(self, overrides: str = None):
        if not self.nodes_setup:
            print("Nodes not set up. Call setup_nodes() before starting.")
            return
        if self.running:
    ...
    def follow_logs(self, host, service: str = None):
        ...
        if not service:
            print(f"No service found for {host.name}")
            return
        ...
        try:
            proc = host.popen(["tail", "-f", log_file], stderr=subprocess.STDOUT, text=True)
    ...
    def _stop_all(self):
        ...
        for host, services in self.running_services.items():
            for service_name, pid in services:
                ...
                print(f"Stopped {service_name} on {host.name}")
    ...
    def _start_service(self, host, cmd, name):
        ...
        print(f"Started {name} on {host.name} with PID {pid.strip()}")
        ...
        def _monitor_serverapp():
            ...
            while self.running:
                ...
                if not self._is_running(self.fl_server, pid):
                    print("\nflwr-serverapp completed. Stopping experiment...")
